...
:::{.callout-note collapse="true" title="Optional (only if you use Git / GitHub)"}
...
:::{.callout-note title="Optional (only if you use Git / GitHub)" collapse="true"}
...
Outsource your preprocessing steps (and especially the heavy computation) into a seperate R-Script called `preprocessing.R`. Iteratively develop this script to import, clean an prepare your data. Then save your result to a file (e.g. as a geopackage, rda or csv file). 
...
If you want to visualize a dataframe as a table, you can using the function `knitr::kabel(df)` (replace `df` with the name of your `data.frame`). For more complex tables, consider [`kableExtra`](https://haozhu233.github.io/kableExtra/) or [`gt`](https://gt.rstudio.com/).
...
:::{.callout-note collapse="true" title="Optional (only if you use Git)"}
...
1. Copy the bibtex key from the paper you want to cite, e.g using google scholar [@fig-scholar]
2. Create a file named `bibliography.bib` and paste the bibtex key from the previous step (see @lst-bibtex). You can change the entry label if you want (e.g. `laube2011`). You can add as many bibtex keys to this file as you like
3. Add `bibliography.bib` to the YAML header of `index.qmd` under the entry `bibliography:` (see @lst-yaml)
4. Now, you can cite Patricks paper anywhere in your text, e.g. using `@laube2011`.

For more citation methods, see <https://quarto.org/docs/authoring/citations.html>


:::{#fig-scholar layout-nrow=1}

![Click on "Zitieren"](images/scholar-1.png){#fig-scholar-1}

![Click on "Bibtex"](images/scholar-2.png){#fig-scholar-2}
...
If you are up for some advanced aspects of writing reports with Quarto, checkout Journal Article Template: <https://quarto.org/docs/journals/formats.html>
